...
async function emitVessels() {
  ...
  await weatherStations
    .find({ timeReceived: { $gte: new Date().getTime() - 60000 * 10 } })
    .forEach((vessel) => {
...
async function updateVessel(vessel) {
  if (vessel.messageType == 5) {
    await vesselNames.updateOne(
      { MMSI: vessel.MMSI },
      { $set: vessel },
      { upsert: true }
    );
  } else if (vessel.messageType == 8 && vessel.dac == 1 && vessel.fid == 11) {
    await weatherStations.updateOne(
      { stationId: vessel.stationId },
      { $set: vessel },
      { upsert: true }
    );
  } else {
...
socket.bind(3000, "127.0.0.1", () => {
  console.log("UDP socket formed on port 3000");
});
